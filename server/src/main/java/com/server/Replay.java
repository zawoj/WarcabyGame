...
import com.board.ChineseCheckersBoardAdapter;
...
public class Replay {
    ...
    public Replay(List<String> logins, List<Integer> moveX, List<Integer> moveY, List<Integer> pawnX, List<Integer> pawnY) {
        this.logins = logins;
        this.moveX = moveX;
        this.moveY = moveY;
        this.pawnX = pawnX;
        ...
        scene.setOnKeyPressed((keyEvent -> {
            if(keyEvent.getCode().equals(KeyCode.RIGHT)){
                if(currentMove!=pawnX.size()-1){
                    try {
                        adapter.move(pawnX.get(currentMove), pawnY.get(currentMove), moveX.get(currentMove), moveY.get(currentMove));
                        currentMove++;
                    } catch (Exception ignored) {
                    }
                }
                ...
                if(currentMove!=0){
                    try {
                        currentMove--;
                        adapter.move(moveX.get(currentMove), moveY.get(currentMove), pawnX.get(currentMove), pawnY.get(currentMove));
                    } catch (Exception ignored) {
                    }
                }
            }
