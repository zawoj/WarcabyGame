...
public class ChineseCheckersBoard {
    ...
    public int checkIfGameEnded() {
        ...
        for (int i = 1; i < size; i++) {
            for (int j = 0; j < i; j++) {
                trianglesize++;
                if (board[height - i][goodPoint + 2 * j] == 4)
                    countInTriangles[3]++;
                if (board[i - 1][goodPoint + 2 * j] == 1)
                    countInTriangles[0]++;
            }
            goodPoint = goodPoint - 1;
        }
        ...
        for (int i = size; i < 2 * size - 1; i++) {
            for (int j = 0; j < size - (i - size) - 1; j++) {
                if (board[height - i][goodPoint + 2 * j] == 5)
                    countInTriangles[4]++;
                if (board[i - 1][goodPoint + 2 * j] == 6)
                    countInTriangles[5]++;
                if (board[i - 1][width - goodPoint - 2 * j - 1] == 2)
                    countInTriangles[1]++;
                if (board[height - i][width - goodPoint - 2 * j - 1] == 3)
                    countInTriangles[2]++;
            }
            goodPoint = goodPoint + 1;
        }
        for (int i = 0; i < 6; i++) {
            if (countInTriangles[i] == trianglesize)
                return i + 1;
        }
        return 0;
    }
